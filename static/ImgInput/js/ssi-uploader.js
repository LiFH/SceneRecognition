            ...
            if (thisS.options.preview) {
                var getTemplate = function (content) {
                    return '<table class="ssi-imgToUploadTable ssi-pending">' +
                     ...
                     '<tr><td><div id="prob1_' + index + '" class="ssi-hidden ">1</div></td></tr>' +
                        '<tr><td><div id="prob2_' + index + '" class="ssi-hidden ">2</div></td></tr>' +
                        '<tr><td><div id="prob3_' + index + '" class="ssi-hidden ">3</div></td></tr>' +
                        '<tr><td><div id="prob4_' + index + '" class="ssi-hidden ">4</div></td></tr>' +
                        '<tr><td><div id="prob5_' + index + '" class="ssi-hidden ">5</div></td></tr>' +
                        '</table>'
                };
        ...
        function ajaxLoopRequest(formData, ii) {
            ...
            $.ajax(ajaxOptions).done(function (responseData, textStatus, jqXHR) {
                ...
                try {
                    ...
                    for(var i=0;i<5;i++){
                        var prob = thisS.$element.find('#prob'+(i+1)+'_' + ii);//get the file's  prob bar
                    prob.removeClass('ssi-hidden') //make it visible
                    console.log(responseData['result']['prediction'][i]['classes']);

                    prob.html(responseData['result']['prediction'][i]['classes']+"|"
                        +responseData['result']['prediction'][i]['probs']);
                    }
